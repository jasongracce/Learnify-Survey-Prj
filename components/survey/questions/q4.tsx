...
export default function Q4({ form, setForm }: Props) {
  const { t } = useLanguage();
  const q = t.surveyPage.step2.q4;

  const toggle = (key: (typeof Q4_OPTIONS)[number]) => {
    setForm((prev) => {
      const has = prev.q4_frustrations.includes(key);
      return {
        ...prev,
        q4_frustrations: has
          ? prev.q4_frustrations.filter((k) => k !== key)
          : [...prev.q4_frustrations, key],
      };
    });
  };
      ...
      <div className="flex flex-col gap-3">
        {Q4_OPTIONS.map((key) => (
          ...
          >
            <input
              type="checkbox"
              checked={form.q4_frustrations.includes(key)}
              onChange={() => toggle(key)}
        ...
        {form.q4_frustrations.includes("other") && (
          <input
            type="text"
            maxLength={200}
            value={form.q4_other}
